...
describe('HTTP server', () => {
  ...
  describe('when PUT /authentications', () => {
    ...
    it('should return 400 payload not contain refresh token', async () => {
      ...
      expect(response.statusCode).toEqual(400);
      expect(responseJson.status).toEqual('fail');
      expect(responseJson.message).toEqual('refresh token tidak boleh kosong');
    });
    ...
    it('should return 400 if refresh token not registerd', async () => {
      // Arrange
      const server = await createServer(container);
      const refreshToken = await container.getInstance(
        TokenManager.name,
      ).createRefreshToken({ username: 'test' });
